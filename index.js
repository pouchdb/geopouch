    ...
    function emit(key, val) {
      ...
      if (options.include_docs) {
        ...
        if (val && typeof val === 'object' && val._id){
          db.get(val._id).then(function(joined_doc){
...
function httpQuery(db, location, opts) {
...
exports.spatial = function (fun,opts,callback){
  ...
  if (typeof fun !== 'string') {
    ...
    result = spatialQuery(this, fun, opts);
  }
...
function spatialQuery(db, fun, opts) {
  if (db.type() === 'http') {
  ...
  return db.get('_design/' + parts[0]).then(function(doc) {
    return viewQuery(db, doc.spatial[parts[1]], opts);
  });
}
...
function calculateBbox(coords){
  return rotateCoords(calculatebounds(coords));
}
